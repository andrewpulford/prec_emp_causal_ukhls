...
# 6-06 - IPTW multiple imputed outcome model (Poor self-rated health) - full-time only 
# Andrew Pulford
...
weightit_mods_srh <- with(data = weightit_df, 
                          exp = glmmTMB(srh_bin_t1 ~
                          # exposure
                          exposure1 +
                            # t0 outcome measure
                            srh_bin_t0 +
...
write.csv(weightit_pooled_srh_df, "./working_data/mi/weightit_pooled_srh_df_ft.csv")
