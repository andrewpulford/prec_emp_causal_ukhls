...
# 5-03 -  Descriptive analysis for IPTW multiple imputed outcome models  - alternate exposure definition
# Andrew Pulford
...
tiff("./output/mi/mi_descriptives/iptw_love_plot_exp2.tiff", width = 980)
...
pdf("./output/mi/mi_descriptives/iptw_love_plot_exp2.pdf", width = 980, height = 980)
love.plot(weightit_df, thresholds = 0.1, stats = "m",
          drop.distance = TRUE, binary = "std", continuous = "std",
          sample.names = c("Unweighted","Inverse probability of treatment weighted")) 
# var.names() - to clean up names
...
write.csv(table_one_sav, "./output/mi/sensitivity_analyses/exposure2/weighted_descriptives/table_one_unpooled_exp2.csv")
...
write.csv(outcomes_desc_sav, "./output/mi/sensitivity_analyses/exposure2/weighted_descriptives/outcomes_desc_exp2.csv")

################################################################################
#####                           sort out table 1                           #####
################################################################################

table_one_sav <- read.csv("./output/mi/sensitivity_analyses/exposure2/weighted_descriptives/table_one_unpooled_exp2.csv")
...
## trim whitespace at both ends of strings
table_one_sav$exposed..no.job.loss.between.t0.and.t1 <- trimws(table_one_sav$exposed..no.job.loss.between.t0.and.t1)

### extract first part of string
table_one_sav$exposed_temp <- sub("\\ .*","",table_one_sav$exposed..no.job.loss.between.t0.and.t1)
## remove commas to allow conversion to numeric
...
## extract second part of string
table_one_sav$exposed_temp2 <- sub("^[^ ]+.","",table_one_sav$exposed..no.job.loss.between.t0.and.t1)
...
table_one_sav$exposed..no.job.loss.between.t0.and.t1 <- paste(table_one_sav$exposed_temp,table_one_sav$exposed_temp2)

## keep only required cols
table_one_sav  <-  table_one_sav %>% dplyr::select(c("X", "level", "unexposed", "exposed..no.job.loss.between.t0.and.t1"))

write.csv(table_one_sav, "./output/mi/sensitivity_analyses/exposure2/weighted_descriptives/table_one_PAPER.csv")
