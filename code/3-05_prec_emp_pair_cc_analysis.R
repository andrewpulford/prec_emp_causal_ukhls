...
#### SF-12 PCS -----------------------------------------------------------------
start_time <- Sys.time()
dr_pcs_glmmTMB_mod <- glmmTMB(sf12pcs_dv_t1 ~ exposure1 +
                              sex_dv_t0 +
                              age_dv_t0 +
                              age_dv_t1 +
                              non_white_t0  +
...
dr_pcs_glmmTMB_mod_summary <- summary(dr_pcs_glmmTMB_mod)

dr_pcs_glmmTMB_df <- tidy(dr_pcs_glmmTMB_mod)

## confidence intervals
dr_pcs_glmmTMB_df_ci <- data.frame(confint(dr_pcs_glmmTMB_mod)) %>% 
  rename(lci = X2.5..,
         uci = X97.5..)

# add in row names
dr_pcs_glmmTMB_df_ci <- cbind(rownames(dr_pcs_glmmTMB_df_ci),dr_pcs_glmmTMB_df_ci, row.names=NULL)

dr_pcs_glmmTMB_df_ci <- dr_pcs_glmmTMB_df_ci %>% 
  rename(term = `rownames(dr_pcs_glmmTMB_df_ci)`)

## join dfs together
dr_pcs_glmmTMB_df <- dr_pcs_glmmTMB_df %>% 
  left_join(dr_pcs_glmmTMB_df_ci) %>% 
  mutate(term = str_remove(term, "exposure1"),
         outcome = "SF-12 PCS",
         est_type = "coefficient",
         p.value = ifelse(p.value<0.001,"<0.001",
...
dr_mcs_glmmTMB_mod <- glmmTMB(sf12mcs_dv_t1 ~ exposure1 +
                              sex_dv_t0 +
                              age_dv_t0 +
                              age_dv_t1 +
                              non_white_t0  +
...
dr_mcs_glmmTMB_summary <- summary(dr_mcs_glmmTMB_mod)

dr_mcs_glmmTMB_df <- tidy(dr_mcs_glmmTMB_mod)

## confidence intervals
dr_mcs_glmmTMB_df_ci <- data.frame(confint(dr_mcs_glmmTMB_mod)) %>% 
  rename(lci = X2.5..,
         uci = X97.5..) #%>% 
#  mutate(lci = exp(lci),
#         uci = exp(uci))
...
# add in row names
dr_mcs_glmmTMB_df_ci <- cbind(rownames(dr_mcs_glmmTMB_df_ci),dr_mcs_glmmTMB_df_ci, row.names=NULL)

dr_mcs_glmmTMB_df_ci <- dr_mcs_glmmTMB_df_ci %>% 
  rename(term = `rownames(dr_mcs_glmmTMB_df_ci)`)

## join dfs together
dr_mcs_glmmTMB_df <- dr_mcs_glmmTMB_df %>% 
  left_join(dr_mcs_glmmTMB_df_ci) %>% 
  mutate(term = str_remove(term, "exposure1"),
         outcome = "SF-12 MCS",
         est_type = "coefficient",
         p.value = ifelse(p.value<0.001,"<0.001",
...
dr_srh_glmmTMB_mod <- glmmTMB(srh_bin_t1 ~ exposure1 +
                              sex_dv_t0 +
                              age_dv_t0 +
                              age_dv_t1 +
                              non_white_t0  +
...
dr_srh_glmmTMB_summary <- summary(dr_srh_glmmTMB_mod)


## coefficients dataframe
dr_srh_glmmTMB_df <- tidy(dr_srh_glmmTMB_mod) %>% 
  # exponentiate to get ORs
  mutate(estimate = exp(estimate))

## confidence intervals
dr_srh_glmmTMB_df_ci <- data.frame(confint(dr_srh_glmmTMB_mod)) %>% 
  rename(lci = X2.5..,
         uci = X97.5..) %>% 
  mutate(lci = exp(lci),
         uci = exp(uci))
...
# add in row names
dr_srh_glmmTMB_df_ci <- cbind(rownames(dr_srh_glmmTMB_df_ci),dr_srh_glmmTMB_df_ci, row.names=NULL)

dr_srh_glmmTMB_df_ci <- dr_srh_glmmTMB_df_ci %>% 
  rename(term = `rownames(dr_srh_glmmTMB_df_ci)`)

## join dfs together
dr_srh_glmmTMB_df <- dr_srh_glmmTMB_df %>% 
  left_join(dr_srh_glmmTMB_df_ci) %>% 
  mutate(term = str_remove(term, "exposure1"),
         outcome = "Poor self-rated health",
         est_type = "OR",
         p.value = ifelse(p.value<0.001,"<0.001",
...
dr_ghq_glmmTMB_mod <- glmmTMB(ghq_case4_t1 ~ exposure1 +
                              sex_dv_t0 +
                              age_dv_t0 +
                              age_dv_t1 +
                              non_white_t0  +
...
dr_ghq_glmmTMB_summary <- summary(dr_ghq_glmmTMB_mod)


## coefficients dataframe
dr_ghq_glmmTMB_df <- tidy(dr_ghq_glmmTMB_mod) %>% 
  mutate(estimate = exp(estimate))

## confidence intervals
dr_ghq_glmmTMB_df_ci <- data.frame(confint(dr_ghq_glmmTMB_mod)) %>% 
  rename(lci = X2.5..,
         uci = X97.5..) %>% 
  mutate(lci = exp(lci),
         uci = exp(uci))
...
# add in row names
dr_ghq_glmmTMB_df_ci <- cbind(rownames(dr_ghq_glmmTMB_df_ci),dr_ghq_glmmTMB_df_ci, row.names=NULL)

dr_ghq_glmmTMB_df_ci <- dr_ghq_glmmTMB_df_ci %>% 
  rename(term = `rownames(dr_ghq_glmmTMB_df_ci)`)

## join dfs together
dr_ghq_glmmTMB_df <- dr_ghq_glmmTMB_df %>% 
  left_join(dr_ghq_glmmTMB_df_ci) %>% 
  mutate(term = str_remove(term, "exposure1"),
         outcome = "GHQ-12 caseness (4+)",
         est_type = "OR",
         p.value = ifelse(p.value<0.001,"<0.001",
...
dr_reg_df <- dr_pcs_glmmTMB_df %>% 
  bind_rows(dr_mcs_glmmTMB_df,
            dr_srh_glmmTMB_df,
            dr_ghq_glmmTMB_df) %>% 
  filter(term=="exposed (employed at t1)") %>% 
  dplyr::select(-term) %>% 
...
write.csv(dr_reg_df, "./output/weighted_outcomes/cc_double_robust_MSM.csv")
